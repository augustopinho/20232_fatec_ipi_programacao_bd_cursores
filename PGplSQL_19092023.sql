...
BEGIN
	...
	LOOP
		FETCH BACKWARD FROM cur_dlete INTO tupla;
		EXIT WHEN NOT FOUND;
		RAISE NOTICE '%', tupla;
	END LOOP;
...
-- DECLARE
-- 	v_ano INT := 2010;
-- 	v_inscrito INT := 60000000;
-- 	v_youtuber VARCHAR(200);
	
-- 	--vinculado (bound)
-- 	--1.Declaração do cursor
-- 	cur_ano_inscritos CURSOR (ano INT, inscritos INT)
-- 	FOR SELECT youtuber FROM tb_top_youtubers WHERE started >= ano AND subscrivers >= inscritos;
-- BEGIN
-- 	--passagem pela ordem
-- 	--OPEN cur_ano_inscritos(v_ano, inscritos);
-- 	--passagem por nome
-- 	--2. Abertura do cursor
